...
df <- read.csv("data/data.csv") #read data
...
df <- df %>% 
  mutate(
    ID = 1:nrow(df),
    Condition = factor(
      Condition, #set reference category
      levels = c("Single",           
                 "Multi-consistent", 
                 "Multi-inconsistent")
      )
...
results <- function(Predictor, Multi_Condition = "Consistent") {
  ...
  ifelse(
    Multi_Condition == "Consistent",
    #print multi-consistent results
    paste(
      apa_results$estimate$ConditionMulti_consistent,
      strsplit(apa_results$statistic$ConditionMulti_consistent, ",")[[1]][2],
      sep = ", " #take out t-value
      ), 
    #print multi-inconsistent results
    paste(
      apa_results$estimate$ConditionMulti_inconsistent,
...
tidy_results <- Predictors %>% 
  map(~ tidy_df(.x)) %>% #map over all predictors to get tidy results
  bind_rows() %>% 
  mutate(
...
ggsave("figures/Figure1.jpg", p1, 
       width = 7, height = 5, units = "in") #save plot

knitr::include_graphics("figures/Figure1.jpg", dpi = 1200)
```
...
p2 <- raincloud("Single estimate", "Consistent crowd", "Inconsistent crowd")

ggsave("figures/Figure2.jpg", p2, 
       width = 7, height = 5, units = "in") #save plot

knitr::include_graphics("figures/Figure2.jpg", dpi = 1200)
```
